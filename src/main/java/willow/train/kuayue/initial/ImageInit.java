...
public class ImageInit {
    ...
    private static void putImageIn(BogeyStyle style) throws URISyntaxException, IOException {
        ...
        for(BogeySizes.BogeySize size : style.validSizes()) {
            ...
            ResourceLocation location = ResourceLocation.fromNamespaceAndPath(Kuayue.MODID, "textures/gui/bogey_style/" +
    ...
    public static ImageRegex register(String path, @Nullable Integer index) {
        if(!hasFired) {
            ImageRegex regex = new ImageRegex(ResourceLocation.fromNamespaceAndPath(Kuayue.MODID, path));
            needToFire.add(regex);
            return regex;
        }
        ImageRegex regex = register(ResourceLocation.fromNamespaceAndPath(Kuayue.MODID, path), index);
        if(regex == null) {
        ...
        public ImageRegex(ResourceLocation location, InputStream imageStream, boolean fromJar) throws IOException {
            ...
            if(!fromJar) {
                ...
                Minecraft.getInstance().getTextureManager().
                        register(ResourceLocation.fromNamespaceAndPath(location.getNamespace(), location.getPath()),
