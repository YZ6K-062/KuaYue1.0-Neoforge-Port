...
public class ColorScreen extends AbstractWidget {
    ...
    @Override
    protected void renderWidget(GuiGraphics pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        if (!visible) return;
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pDelta) {
        if (!visible) return false;
        boolean flag = selector.mouseScrolled(pMouseX, pMouseY, 0, pDelta);
        if (flag) updateFromSelectorToBox();
        return super.mouseScrolled(pMouseX, pMouseY, 0, pDelta) & flag;
    }
