...
public class ColorScreenBundles extends AbstractWidget {
    ...
    public void renderWidget(GuiGraphics pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        // 编辑态下 templateScreen 自身会被隐藏，但其弹窗仍需绘制（1.21 的 render 是 final，无法覆写转发）
        templateScreen.editScreen.render(pPoseStack, pMouseX, pMouseY, pPartialTick);
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pDelta) {
        if(!visible || !isMouseOver(pMouseX, pMouseY)) return false;
        boolean result = false;
        result = colorEditor.mouseScrolled(pMouseX, pMouseY, 0, pDelta);
        result = result || templateScreen.mouseScrolled(pMouseX, pMouseY, 0, pDelta);
        result = result || templateEditor.mouseScrolled(pMouseX, pMouseY, 0, pDelta);
        return result;
    }
