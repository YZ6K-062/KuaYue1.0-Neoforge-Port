...
public class TypeScreen extends CustomScreen<EditablePanelEditMenu, EditablePanelEntity> {
    ...
    public void init() {
        ...
        confirmBtn.setOnClick((w, x, y) -> {
            ...
            tag.put("data", nbt);
            entity.loadCustomOnly(tag, entity.getLevel().registryAccess());
            entity.markUpdated();
    ...
    private void refresh() {
        ...
        int focus = -1;
        int focusIndex = -1;
        for (AbstractWidget w : getWidgets()) {
        ...
        if (focus > -1 && focusIndex > -1) {
            AbstractWidget w = getWidgets().get(focus);
        ...
        for (AbstractWidget widget : getWidgets()) {
            ...
            if (widget instanceof TransparentEditBox box) {
                editBar.setPosition(box.getX() + ((int) ((float) box.getWidth() * box.getScaleX()) - 200) / 2,
                        box.getY() + (int) ((float) box.getHeight() * box.getScaleY()) + 40);
    ...
    @Override
    public void renderBackGround(GuiGraphics pose, int mouseX, int mouseY, float partialTick) {
        if (Minecraft.getInstance().screen == null) return;
        int sW = Minecraft.getInstance().screen.width;
        int sH = Minecraft.getInstance().screen.height;
        pose.fill( 0, 0, sW, sH, 0x80000000);
    }
