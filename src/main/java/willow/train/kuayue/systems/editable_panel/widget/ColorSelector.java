...
public class ColorSelector extends AbstractWidget {
    ...
    public void renderWidget(GuiGraphics pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        pPoseStack.fill( this.getX() + 54, this.getY() + 54, this.getX() + 86, this.getY() + 86, color.getRGB());
        bg.get().renderToGui();
    ...
    @Override
    public boolean mouseScrolled(double mx, double my, double pScrollX, double pDelta) {
        if (!visible) return false;
