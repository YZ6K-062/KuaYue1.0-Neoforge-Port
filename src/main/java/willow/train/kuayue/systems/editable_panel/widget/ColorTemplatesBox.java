...
public class ColorTemplatesBox extends AbstractWidget {
    ...
    public void renderWidget(GuiGraphics pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (isHovered) {
            renderLines(pPoseStack, this.getX(), this.getY(), this.getX() + width, this.getY() + height,
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pDelta) {
        return super.mouseScrolled(pMouseX, pMouseY, 0, pDelta);
    }
    ...
    public static void renderGuiBg(GuiGraphics poseStack, int minX, int minY, int maxX, int maxY, int borderWidth, int bgColor, int borderColor) {
        // border
        poseStack.fill(minX, minY, maxX, minY + borderWidth, borderColor);
        poseStack.fill(minX, maxY - borderWidth, maxX, maxY, borderColor);
        poseStack.fill(minX, minY, minX + borderWidth, maxY, borderColor);
        poseStack.fill(maxX - borderWidth, minY, maxX, maxY, borderColor);

        // bg
        poseStack.fill(minX + borderWidth, minY + borderWidth, maxX - borderWidth, maxY - borderWidth, bgColor);
    }

    public static void renderLines(GuiGraphics poseStack, int minX, int minY, int maxX, int maxY, int borderWidth, int bgColor, int borderColor) {
        poseStack.fill(minX, minY, maxX, minY + borderWidth, borderColor);
        poseStack.fill(minX, maxY - borderWidth, maxX, maxY, borderColor);

        poseStack.fill(minX, minY + borderWidth, maxX, maxY - borderWidth, bgColor);
    }
